...
# ---- shop -> sink mapping (units/day drained from a product's market) --------
# Each shop instance drains 1 unit / 4 turns = 6 units/day per product it lists;
# single-product shops (YARN_STORE, PET_CAFE) drain 2 units / 4 turns = 12/day.
SHOP_PRODUCTS = {
    "BAKERY":         ["EGG", "WHEAT"],
...
# town center also drains 1 unit/day of every product (except FERTILIZER).
TOWN_CENTER = 1


def shop_sinks(unlocked_shops):
    """Return {product: units/day drained} from the observed unlocked shops."""
    sink = {p: 0 for p in PRODUCTS}
    for shop in unlocked_shops or []:
        prods = SHOP_PRODUCTS.get(shop)
        if not prods:
            continue
        rate = 12 if len(prods) == 1 else 6
        for item in prods:
            sink[item] += rate
    for item in PRODUCTS:
        if item != "FERTILIZER":
            sink[item] += TOWN_CENTER
    return sink
...
# ---- parameters -------------------------------------------------------------
# sol6: fixed animal/crop counts are replaced by shop-demand-sized targets, so
# these knobs are base counts, caps, hire schedule and the sell reservation
# curve. Defaults are the design values from PLAN.md (NOT CEM-tuned).
# (name, lo, hi, seed)
PARAM_SPEC = [
    ("melon_tiles",      8,   16,   14),
    ("base_cows",        1,   3,    2),
    ...
    ("straw_base",       10,  28,   24),
    ("straw_cap",        16,  34,   32),
    ("wheat_tiles",      20,  36,   30),
    ("tomato_tiles",     0,   10,   6),
    ("hands_early",      3,   8,    5),
    ("hands_mid",        5,   12,   8),
    ("hands_late",       8,   14,   11),
    ("share",            0.3, 0.7,  0.5),
    ("reserve_hi",       0.7, 1.0,  0.9),
    ("reserve_decay",    0.4, 0.9,  0.65),
    ("reserve_lo",       0.15, 0.4, 0.25),
    ("taper_start_day",  25,  29,   27),
    ("buy_SE",           0,   1,    0),
    ("drop_threshold",   4,   20,   9),
    ("wheat_buy_buffer", 0,   10,   3),
    ("fert_reserve",     0,   12,   4),
    ("straw_deadline",   6,   16,   15),
]
DIM = len(PARAM_SPEC)
SEED_VEC = [s for (_, _, _, s) in PARAM_SPEC]
FLOAT_PARAMS = {"share", "reserve_hi", "reserve_decay", "reserve_lo", "buy_SE"}
INT_PARAMS = {name for (name, _, _, _) in PARAM_SPEC if name not in FLOAT_PARAMS}
...
def build_agent(params_or_vec):
    P = params_or_vec if isinstance(params_or_vec, dict) and "base_cows" in params_or_vec \
        else vec_to_params(params_or_vec)

    mem = {"day": -1, "last_key": {}, "crew": None, "prev_idle": 0,
           "prev_anim_pos": None, "prev_cull_pos": set(),
           "stats": {"days": {}, "escapes": 0, "escapes_intended": 0, "plant_blocked": 0}}
    ...
    def livestock_targets(day, sinks, prev_idle, mode_by_type):
        """Demand-sized daily targets from observed shop sinks (PLAN 3a).

        cows/sheep scale with the milk/wool sink we expect to capture (share);
        geese are the labor filler sized to the previous day's idle unit-turns.
        When a product is being culled (flooded), keep only the base count.
        """
        share = P["share"]
        cows = P["base_cows"] + round(share * sinks.get("MILK", 0) / 1.5)
        sheep = P["base_sheep"] + round(share * sinks.get("WOOL", 0) / 1.33)
        cows = max(P["base_cows"], min(P["cow_cap"], int(cows)))
        # Cows need ~8 days after placement to yield, so a demand signal that
        # only arrives after the buy deadline is worthless. When a real milk
        # sink is already visible (>=2 milk-shop-equivalents) and it is early
        # enough for the animals to still produce, lean toward the cap -- milk
        # is the most inelastic product, so extra cows keep selling profitably.
        if sinks.get("MILK", 0) >= 12 and day <= 15:
            cows = max(cows, min(P["cow_cap"], 8))
        sheep = max(P["base_sheep"], min(P["sheep_cap"], int(sheep)))
        if mode_by_type.get("COW") == "CULL":
            cows = P["base_cows"]
        if mode_by_type.get("SHEEP") == "CULL":
            sheep = P["base_sheep"]
        egg_sink = sinks.get("EGG", 0)
        if day < 6 or egg_sink <= 2:
            # no egg demand -> geese only steal cash and builder labor from cows
            geese = 0
        else:
            # ~1 egg/day/goose base (plus care bonus); size to captured egg sink
            geese = round(share * egg_sink / 2.0)
            geese = max(0, min(P["goose_cap"], int(geese)))
    ...
    def agent(obs):
        ...
        mkt_prices = market.get("prices", {}) or {}
        unlocked = me.get("unlocked_quadrants", ["NW"])
        shops = (obs.get("town", {}) or {}).get("unlocked_shops", []) or []
        sinks = shop_sinks(shops)
        stats = mem["stats"]

        if day != mem["day"]:
            prev = stats["days"].get(mem["day"])
            mem["prev_idle"] = prev["idle"] if prev else 0
            mem["day"] = day
            mem["last_key"] = {}
            mem["crew"] = None
            stats["days"][day] = {"fed": 0, "cared": 0, "animals": 0, "money_h0": money,
                                  "idle": 0, "moves": 0, "actions": 0, "kept": 0}
        ...
        wheat_price = mkt_prices.get("WHEAT") or market_price("WHEAT", mkt_inv.get("WHEAT", MARKET_I0))
        days_left = max(1, LAST_DAY - day)

        # ---- culling / alternate-day feeding (PLAN 3a/3c) -------------------
        # Per animal type decide whether surplus animals (above the base count
        # kept for fertilizer) are worth feeding. A product that is flooded
        # below 2*wheat and cannot recover before the end is CULLed (surplus
        # animals are not fed -> escape, freeing labor); one that is merely soft
        # (below 0.5*base) is fed on alternate days; otherwise KEEP.
        def _mode(product):
            base_price = MARKET_PARAMS[product]["base"]
            price = mkt_prices.get(product) or market_price(product, mkt_inv.get(product, MARKET_I0))
            sink = sinks.get(product, 0)
            inv = mkt_inv.get(product, MARKET_I0)
            recovery = (inv - MARKET_I0) / sink if sink > 0 else 1e9
            if price < 2 * wheat_price and recovery > days_left:
                return "CULL"
            # NB: no alternate-day ("ALT") mode -- an animal fed only every other
            # day has zero slack against the 2-day escape rule, so a single missed
            # feeder visit loses it. Wheat is cheap; KEEP (feed daily) is safer and
            # only genuinely flooded products are CULLed (intentionally abandoned).
            return "KEEP"
        mode_by_type = {"COW": _mode("MILK"), "SHEEP": _mode("WOOL"), "GOOSE": "KEEP"}
        base_count = {"COW": P["base_cows"], "SHEEP": P["base_sheep"], "GOOSE": 999}
        feed_mode = {}
        for a in ANIMALS:
            insts = sorted([(x, y, t) for (x, y, t) in animals if t["animal"] == a],
                           key=lambda z: (z[2].get("placed_day", 0), z[0], z[1]))
            for i, (x, y, t) in enumerate(insts):
                feed_mode[(x, y)] = "KEEP" if i < base_count[a] else mode_by_type[a]

        def should_feed(x, y, t):
            m = feed_mode.get((x, y), "KEEP")
            if not feed_today:
                return False
            if m == "CULL":
                return False
            if m == "ALT":
                return t.get("consecutive_unfed", 0) >= 1
            return True
        cull_pos = {p for p, m in feed_mode.items() if m == "CULL"}
        dstat["kept"] = sum(1 for (x, y, t) in animals if feed_mode.get((x, y)) != "CULL")
        # feeding-guarantee measurement: among animals we intend to feed TODAY
        # (KEEP always, ALT only when it would otherwise starve), how many are fed.
        dstat["need_feed"] = sum(1 for (x, y, t) in animals if should_feed(x, y, t))
        dstat["fed_need"] = sum(1 for (x, y, t) in animals if should_feed(x, y, t) and t.get("fed_today"))

        # ---- escape accounting (unintended vs intended cull) -----------------
        if mem["prev_anim_pos"] is not None and hour <= 1:
            escaped = mem["prev_anim_pos"] - set(animal_at.keys())
            for p in escaped:
                # a CULL escape takes up to 2 unfed days, so an animal culled on
                # either of the last two snapshots was abandoned intentionally.
                if p in mem["prev_cull_pos"] or p in mem.get("cull_recent", set()):
                    stats["escapes_intended"] += 1
                else:
                    stats["escapes"] += 1
        if hour == 0:
            mem["prev_anim_pos"] = set(animal_at.keys())
            mem["cull_recent"] = set(mem.get("prev_cull_pos", set()))
            mem["prev_cull_pos"] = set(cull_pos)

        # ---- day plan --------------------------------------------------------
        a_target = livestock_targets(day, sinks, mem["prev_idle"], mode_by_type)
        for a in ANIMALS:
        ...
        total_animal_goal = P["cow_cap"] + P["sheep_cap"] + P["goose_cap"]
        slots = animal_slots(unlocked)
        reserved = set(slots[:min(len(slots), max(a_target["COW"] + a_target["SHEEP"] + a_target["GOOSE"] + 2, total_animal_goal // 2))])

        has_tomato_shop = any(s in ("FARMERS_MARKET", "PIZZA_SHOP") for s in shops)
        straw_target = int(max(P["straw_base"], min(P["straw_cap"], P["straw_base"] + round(P["share"] * sinks.get("STRAWBERRY", 0)))))
        crop_target = {
            "MELON": P["melon_tiles"] if day <= 2 else 0,
            "STRAWBERRY": straw_target if 2 <= day <= P["straw_deadline"] else 0,
            "TOMATO": (P["tomato_tiles"] if has_tomato_shop else 0) if 3 <= day <= 16 else 0,
            "WHEAT": P["wheat_tiles"] if day <= 26 else 0,
            "CARROT": 999 if 22 <= day <= 27 else 0,  # tail filler only
        }
        ...
        def animal_pending(t, pos=None):
            ...
            a = ANIMALS[t["animal"]]
            ops = []
            fd = should_feed(pos[0], pos[1], t) if pos is not None else feed_today
            if fd and not t.get("fed_today"):
                ops.append("FEED")
            if fd and not t.get("cared_today"):
                ops.append("CARE")
        ...
        # late-day safety net: unfed animals become everyone's problem. Animals
        # already one unfed day deep (consecutive_unfed>=1) have zero slack, so
        # they are top priority at ALL hours; the rest join in the afternoon.
        atrisk = [(x, y) for (x, y, t) in animals
                  if not t.get("fed_today") and should_feed(x, y, t)
                  and t.get("consecutive_unfed", 0) >= 1]
        for (x, y) in atrisk:
            add(11, x, y, ["FEED"], need="WHEAT")
        if feed_today and hour >= 14:
            unfed_all = [(x, y) for (x, y, t) in animals
                         if not t.get("fed_today") and should_feed(x, y, t)]
            if unfed_all:
                for (x, y) in unfed_all:
                    if (x, y) not in atrisk:
                        add(10, x, y, ["FEED"], need="WHEAT")
        picks = atrisk if hour < 14 else [(x, y) for (x, y, t) in animals
                     if not t.get("fed_today") and should_feed(x, y, t)]
        if feed_today and picks and carried.get("WHEAT", 0) < len(picks) and shed.get("WHEAT", 0) > 0:
            add(11 if atrisk else 10, None, None,
                ["PICKUP", "WHEAT", min(8, shed.get("WHEAT", 0), len(picks))],
        ...
        for (x, y, t) in plants:
            ...
            if cd["ongoing"] and not decaying and t.get("fertilized_until_day", -1) < day and day <= 26:
                # Fertilize on each production cycle: strawberry produces on
                # ages 10/12/14 (fert active for day..day+2 covers them).
                want = (c == "STRAWBERRY" and age in (9, 11, 13)) or (c == "TOMATO" and age in (7, 10))
                if want:
                    add(P_FERTILIZE, x, y, ["FERTILIZE"], need="FERTILIZER")
                    fert_tasks += 1
            # WHEAT: fertilize at age 2 -> 6 units/tile instead of 4 (+2u ~= $70).
            # Lower priority than strawberry fert so scarce fertilizer goes to
            # strawberry first; only when fertilizer is plentiful.
            elif (c == "WHEAT" and age == 2 and t.get("fertilized_until_day", -1) < day
                  and day <= 24 and yu < cd["maxy"]
                  and (shed.get("FERTILIZER", 0) + carried.get("FERTILIZER", 0)) > 6):
                add(P_PICK_FERT, x, y, ["FERTILIZE"], need="FERTILIZER")
                fert_tasks += 1
        ...
        # ---- market orders -----------------------------------------------------
        orders = []
        unfed_n = sum(1 for (x, y, t) in animals
                      if feed_today and should_feed(x, y, t) and not t.get("fed_today"))
        kept_animals = sum(1 for (x, y, t) in animals if feed_mode.get((x, y)) != "CULL")
        animals_total = n_animals + sum(shed.get(a, 0) + carried.get(a, 0) for a in ANIMALS)
        # wheat reserve excludes culled animals (they are meant to escape).
        wheat_reserve = unfed_n + (kept_animals if day < LAST_DAY - 1 else 0) + P["wheat_buy_buffer"]
        overflow = shed_total + carried_total > 80
        taper = day >= P["taper_start_day"]
        final_dump = last_day and hour >= 19

        def reserve_frac_now():
            return max(P["reserve_lo"], P["reserve_hi"] - P["reserve_decay"] * (day - 10) / 19.0)

        def sell_qty(item):
            # PLAN 3d/E: uncrashable products (EGG, WHEAT-above-reserve) and the
            # finite pies (MELON, FERTILIZER-above-use) sell fully every turn;
            # sink products sell all while the market is below I0 (scarcity
            # premium), and above I0 sell what the sink drains before the next
            # turn plus the units still clearing a reservation price that decays
            # with days left; always liquidate what cannot recover before the end.
            q = shed.get(item, 0)
            if q <= 0:
                return 0
            if final_dump:
            ...
            if item == "EGG":
                return q
            if item == "WHEAT":
                return q if overflow else max(0, q - wheat_reserve)
            if item == "FERTILIZER":
                n_ongoing = crop_count.get("STRAWBERRY", 0) + crop_count.get("TOMATO", 0)
                keep = 0 if (day >= 27 or money < 300) else min(P["fert_reserve"],
                                                                 n_ongoing + (1 if crop_count.get("WHEAT", 0) else 0))
                q2 = max(0, q - keep)
                return sellable_qty(item, inv, 0.15, q2) if q2 > 0 else 0
            # ---- sink products: STRAWBERRY, MILK, WOOL, TOMATO, CARROT --------
            sink = sinks.get(item, 0)
            if inv < MARKET_I0:
                k = q                                       # scarcity premium
            elif sink <= 0 or (inv - MARKET_I0) / sink > days_left:
                k = sellable_qty(item, inv, P["reserve_lo"], q)   # cannot recover -> dump above floor
            else:
                cap = 8 if item in FRAGILE else 10
                k = sellable_qty(item, inv, reserve_frac_now(), min(q, cap))
                sink_turn = max(1, math.ceil(sink / TURNS_PER_DAY))
                k = min(q, k + sink_turn)
            # pressure valve: never end the day holding more than ~next day's sink
            hold = max(6, min(sink, 12)) if item in FRAGILE else 15
            if stock > hold:
                k = max(k, min(q, math.ceil((stock - hold) / 3)))
            if taper:
                turns = max(1, endgame_turns_left + 1)
                k = max(k, min(q, math.ceil(stock / turns)))
                if endgame_turns_left < 48:
                    frac = endgame_turns_left / 48.0
                    thr2 = P["reserve_lo"] + (P["reserve_hi"] - P["reserve_lo"]) * frac
        ...
        # early melon seeds: a first-mover finite pie that MUST be planted by
        # day 1, so fund it ahead of animals (which can be bought later; cow
        # deadline is day 19). Buys the full remaining deficit, not the 10/turn cap.
        if not last_day and day <= 1 and len(orders) < 10:
            want = crop_target["MELON"] - crop_count["MELON"] - seeds.get("MELON", 0)
            n = min(max(0, want), int(max(0, money - reserve_cash) // CROPS["MELON"]["seed"]))
            if n > 0:
                orders.append(["BUY_SEED", "MELON", n])
                money -= n * CROPS["MELON"]["seed"]
