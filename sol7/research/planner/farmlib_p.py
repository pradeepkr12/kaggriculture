"""sol7 research copy of sol5/farmlib.py with planner hooks (see planner.py).

Hooks (all read from P at decision time, so a controller can mutate P in place):
  P["planner"]      -> True: animal targets come straight from n_cows/n_sheep/n_geese (no day caps)
  P["cull"]         -> {animal_type: n}: stop FEED/CARE on n animals of that type (they escape after 2 days)
  P["sell_override"]-> callable(item, q, ctx) -> int or None (None = fall back to sol5 rule)
  P["hands"]        -> optional callable(day) -> hands target

Original docstring: sol4 parametrized policy: animal-core + throttled multi-market farm.
...
def build_agent(params_or_vec):
    ...
    def agent(obs):
        ...
        # planner cull: the first n animals of a type (by slot order) are no longer fed/cared
        cull_set = set()
